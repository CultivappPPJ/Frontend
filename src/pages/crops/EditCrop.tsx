...
export default function EditCrop() {
  const { id } = useParams();
  const { handleSubmit, control, setValue, reset } = useForm<IFormCrop>();
  ...
  const onSubmit: SubmitHandler<IFormCrop> = async (data) => {
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_UPDATE_CROP}/${id}`,
        terrainData,
        {
          headers: {
      ...
      console.error("Ocurrió un error al actualizar un cultivo: ", error);
    } finally {
      ...
      if (id) {
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_FIND_CROP_BY_ID}/${id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          Object.keys(data).forEach((key) => {
            if (key in data) {
              setValue(key as keyof IFormCrop, data[key]);
            }
          });
        } catch (error) {
          console.error(
